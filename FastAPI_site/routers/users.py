...
async def all_users(db: Annotated[Session, Depends(get_db)]):
    """GET функция. Возвращает список всех пользователей"""
...
async def create_user(username: str = Form(), email: str = Form(), password: str = Form(),
    ...
    """POST функция. Регистрирует новых пользователей"""
    try:
...
async def login(request: Request):
    """GET функция. Обрабатывает страницу аутентификации"""
    return templates.TemplateResponse('users/login.html', {'request': request})


@router.post('/login')
async def login_user(username: str = Form(), password: str = Form(), db: Session = Depends(get_db)):
    """POST функция. Аутентифицирует пользователя через форму"""
    try:
...
async def update_user(
    ...
    """PUT функция. Обновляет данные пользователя по ID"""
...
async def delete_user(
    ...
    """DELETE функция. Удаляет пользователя по ID"""
